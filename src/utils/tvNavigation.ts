...
export function handleTvRemoteNavigation(
  ...
  // When focused on an input/textarea, NEVER hijack Backspace or Delete — let user edit text naturally
  if (isInputFocused && (key === 'Backspace' || keyCode === 8 || key === 'Delete')) {
    return false;
  }

  // 1. Detect Back / Return button (Escape, Android Back, Tizen 10009, WebOS 461)
  // Only treat Backspace/keyCode 8 as back button if NOT focused in an input
  const isBackKey =
    key === 'Escape' ||
    key === 'Back' ||
    key === 'GoBack' ||
    ...
    keyCode === 461 ||
    keyCode === 10009 ||
    (!isInputFocused && (key === 'Backspace' || keyCode === 8));
